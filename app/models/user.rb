...
:avatar
  ...
  validates_presence_of :email, :username, :state
	...
	def some_feeds
		Feed.find(:all, :conditions => ["user_id in (?)", Friendship.map(&:id).push(self.id)], :order => "created_at desc", :limit => 8)
	end

	def all_feeds
		Feed.find(:all, :conditions => ["user_id in (?)", Friendship.map(&:id).push(self.id)], :order => "created_at desc")
	end

	def friend_events
		Event.find(:all, :conditions => ["user_id in (?)", Friendship.map(&:id)], :order => "created_at desc")
	end
